"""
================================================================================
AREA199 BASKET LAB — FOGLIO DI CAMPO STAMPABILE
================================================================================
Genera un documento HTML pronto da stampare, con una griglia per ogni test
selezionato e le righe di tutti gli atleti della rosa.
...
def _intestazione_prove(col: str) -> list[str]:
    """Colonne da stampare per un test, in base al numero di prove previste."""
...
def _tabella_test(col: str, atleti) -> str:
    ...
    for i, (_, a) in enumerate(atleti.iterrows(), start=1):
        ...
        righe += (f'<tr><td class="n">{i}</td>'
                  ...
                  f'<td class="r">{a["ruolo"][:3].upper()}</td>{celle}</tr>')
...
.testata {{ display: flex; justify-content: space-between; align-items: flex-end;
            border-bottom: 3px solid #C9A227; padding-bottom: 9px; margin-bottom: 14px; }}
.marchio {{ font-size: 21px; font-weight: bold; letter-spacing: 2px; }}
.marchio small {{ display: block; font-size: 9px; font-weight: normal;
                  letter-spacing: 3px; color: #666; margin-top: 3px; }}
...
.avviso {{ background: #FAF6E8; border-left: 3px solid #C9A227;
           padding: 8px 11px; margin-bottom: 15px; font-size: 10px; line-height: 1.5; }}
...
.proto {{ font-size: 9.5px; color: #444; line-height: 1.45;
...
td.n {{ width: 22px; text-align: center; font-size: 10px; color: #777;
        padding: 3px; }}
td.a {{ width: 150px; padding: 3px 6px; font-size: 11px; }}
td.r {{ width: 42px; text-align: center; font-size: 9px; color: #555; padding: 3px; }}
...
.pie {{ margin-top: 16px; text-align: center; font-size: 8px; color: #888;
...
<div class="testata">
  <div class="marchio">AREA199<small>BIOMECHANICAL LAB</small></div>
  <div class="dati">
    <b>FOGLIO DI RILEVAZIONE — {sessione}</b><br>
    {squadra or 'Squadra'} · {data_test.strftime('%d/%m/%Y')}<br>
    Atleti in elenco: {len(atleti)}
  </div>
</div>
...
<div class="pie">
  AREA199 — Biomechanical Lab · Dott. Antonio Petruzzi ·
